...
In buona sostanza si tratta di un tool che prende in input un portlet project e restituisce in output, nella cartella build, tutti i file necessari, compresi gli npm packages, così che la gradle build standard liferay, possa produrre un OSGI Bundle.
...
In fase di build il bundler deve compiere delle operazioni primarie:
...
Queste operazioni consentono che in fase di deploy liferay possa trattare correttamente i moduli javascript
